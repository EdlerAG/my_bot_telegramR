...
class Database:
    ...
    @staticmethod
    async def get_reminders_cleanup_stats(days=0):
        """Повертає статистику для /db_clean і /db_status."""
        async with aiosqlite.connect(DB_NAME, timeout=30) as db:
            ...
            async with db.execute("SELECT COUNT(*) FROM reminders WHERE status='spamming'") as c:
                spamming = (await c.fetchone())[0]

            async with db.execute("SELECT COUNT(*) FROM reminders WHERE status NOT IN ('pending','spamming')") as c:
                done_or_inactive = (await c.fetchone())[0]

            async with db.execute("SELECT COUNT(*) FROM reminders WHERE status='pending' AND remind_time < datetime('now')") as c:
                overdue_pending = (await c.fetchone())[0]

            older_than_days = 0
            if days > 0:
                async with db.execute(
                    "SELECT COUNT(*) FROM reminders WHERE status != 'pending' AND remind_time < datetime('now', ?)",
                    (f'-{days} days',)
                ) as c:
                    older_than_days = (await c.fetchone())[0]

            return {
                "total": total,
                "pending": pending,
                "spamming": spamming,
                "done_or_inactive": done_or_inactive,
